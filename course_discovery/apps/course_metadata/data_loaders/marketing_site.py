...
from course_discovery.apps.course_metadata.models import (
    Course, Organization, Person, Subject, Program, Position, LevelType, CourseRun, CanonicalCourseRunCourse
)
...
class CourseMarketingSiteDataLoader(AbstractMarketingSiteDataLoader):
    ...
    def process_node(self, data):
        ...
        if course_run:
            ...
            if course_run.canonical_course:
                course = self.update_course(course_run.canonical_course, data)
            ...
            if created:
                CanonicalCourseRunCourse.objects.create(course=course, course_run=course_run)
    ...
    def update_course_run(self, course_run, data):
        validated_data = self.format_course_run_data(data)
    ...
    def create_course_run(self, course, data):
        defaults = self.format_course_run_data(data, course=course)
    ...
    def get_or_create_course(self, data):
        ...
        defaults = self.format_course_data(data)

        return Course.objects.get_or_create(key__iexact=key, partner=self.partner, defaults=defaults)
    ...
    def format_course_run_data(self, data, course=None):
        uuid = data['uuid']
    ...
    def format_course_data(self, data):
        defaults = {
